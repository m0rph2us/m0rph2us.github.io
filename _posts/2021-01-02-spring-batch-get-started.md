...
배치란 사전적인 의미 그대로 무언가를 일괄적으로 처리함을 뜻한다. 개발자들 입장에서 보면 어딘가에 저장된 데이터를 일괄적으로 읽어 처리하고 어떠한 액션을
...
다음 샘플 코드는 코틀린으로 작성되었으며, 1부터 25까지의 수에 3을 곱하는 아주 단순한 예제이다. 구현 내용은 단순하지만 기본 구조는 다른 배치 
애플리케이션을 작성하는 경우에도 크게 바뀌지 않기 때문에 잘 봐두기 바란다.
...
{:refdef: style="text-align: center;"}
![chunk_oriented_processing](/assets/chunk-oriented-processing.png)
{:refdef}
